...
import cimgui.internal.ImGuiInputTextCallbackData
import cimgui.internal.ImGuiSizeCallbackData
import kotlin.reflect.*


internal inline fun <T> withCallback(
	noinline callback: ImGuiInputTextCallback? = null,
	block: (actualCallback: InputTextCallback?) -> T
): T {
...
actual fun ImGui.plotHistogram(
	...
	values.nativeCopy { valuesPinned ->
		usingVec2(graphSize) { graphSizePtr ->
			CImGui.igPlotHistogramFloatPtr(
				label,
				valuesPinned,
				values.size,
				valuesOffset,
...
actual fun ImGui.plotHistogram(
	...
	return usingVec2(graphSize) { graphSizePtr ->
		CImGui.igPlotHistogramFnFloatPtr(
			label,
			getter,
			valuesCount,
			valuesOffset,
