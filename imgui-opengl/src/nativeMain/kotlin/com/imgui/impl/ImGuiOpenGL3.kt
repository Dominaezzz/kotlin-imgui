...
actual class ImGuiOpenGL3 actual constructor(glslVersionStr: String) : Closeable {
	...
	init {
		glVersion =
			if (!glIsOpenGLES) glGetInteger(GL_MAJOR_VERSION) * 100 + glGetInteger(GL_MINOR_VERSION) * 10 else 200
		...
		io.BackendRendererUserData = StableRef.create(this).asCPointer()
		...
		io.BackendFlags = io.BackendFlags or ImGuiBackendFlags.RendererHasViewports.value
		...
		val arena = Arena()
		try {
			val pixels = arena.allocPointerTo<UByteVar>()
			val width = arena.alloc<IntVar>()
			val height = arena.alloc<IntVar>()
	...
	actual fun renderDrawData(drawData: ImDrawData) {
		...
		val clipOriginLowerLeft = if (glHasClipOrigin) {
		...
		for (n in 0 until drawData.cmdListsCount) {
			...
			glBufferData(
				...
				cmdList.IdxBuffer.Size * sizeOf<ImDrawIdxVar>(),
					...
					if (x < fbWidth && y < fbHeight && z >= 0 && w >= 0.0f) {
						...
						if (glMayHaveVertexOffset) {
							glDrawElementsBaseVertex(
								GL_TRIANGLES,
								pcmd.ElemCount.toInt(),
								indexType,
								indexOffset,
								pcmd.VtxOffset.toInt()
							)
						} else {
		...
		glBlendFuncSeparate(
			lastBlendSrcRGB.toUInt(),
			lastBlendDstRGB.toUInt(),
			lastBlendSrcAlpha.toUInt(),
			lastBlendDstAlpha.toUInt()
		)
